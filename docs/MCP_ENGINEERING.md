...
> 这部分将写入 MANUAL.md 对外发布，帮助用户快速上手。
...
async def call_tool(name: str, arguments: dict):
    if name == "search_docs":
        results = my_search_function(arguments["query"])  # 替换成你的逻辑
...
async def list_tools():
    return [
        ...
        Tool(
            name="chat",
            description="与该 Agent 进行对话，获取专业建议",
            inputSchema={
                "type": "object",
                "properties": {
                    "messages": {
...
async def call_tool(name: str, arguments: dict):
    if name == "chat":
        ...
        reply = my_agent_logic(messages)  # 替换成你的对话逻辑
        return [TextContent(type="text", text=reply)]
```

平台接收到协作任务时，会以 `{"messages": [...]}` 格式调用这个工具，把你的 Agent 视为对话参与者。
